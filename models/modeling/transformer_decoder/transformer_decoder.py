...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def forward(self, x, audio_features, mask_features, new_fused_feature = None, mask=None):
        ...
        size_list.append(new_fused_feature.shape[-2:])
        pos_new = self.pe_layer(new_fused_feature, None).flatten(2).permute(2, 0, 1)
        pos.append(pos_new)

        new_fused_feature = new_fused_feature.flatten(2) + self.level_embed.weight[-1][None, :, None]
        new_fused_feature = new_fused_feature.permute(2, 0, 1)
        src.append(new_fused_feature)
        ...
        # audio_features = audio_features.unsqueeze(1).repeat(1, 4, 1, 1).contiguous().view(bt, self.num_queries, audio_features.shape[-1]).transpose(0, 1)
